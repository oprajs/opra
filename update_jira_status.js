...
async function getResponse(){
  try {
    ...
    const githubResponse = await axios.get(`https://api.github.com/repos/${GITHUB_REPOSITORY}/issues/${GITHUB_ISSUE_NUMBER}`, {
      headers: {
    ...
    if (githubIssueClosed) {
      ...
      if (jiraResponse.data && Array.isArray(jiraResponse.data.issues)) {
        ...
        const doneTransition = transitionsResponse.data.transitions.find(transition => transition.name.toLowerCase() === 'done');
        if (doneTransition) {
