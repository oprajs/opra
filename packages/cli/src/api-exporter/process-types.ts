 ...
 * @param dataType
 */
export async function generateTypeFile(this: ApiExporter, dataType: DataType): Promise<TsFile | undefined> {
  ...
  let filePath = '';
  if (dataType instanceof SimpleType) filePath = '/simple-types.ts';
  else {
    if (dataType instanceof EnumType) filePath = path.join('/enums', typeName + '.ts');
    else filePath = path.join('/types', typeName + '.ts');
  }
...
export async function resolveTypeNameOrDef(
  ...
  if (dataType instanceof SimpleType) return this.generateSimpleTypeDefinition({ ...args, dataType });
  if (dataType instanceof EnumType) return this.generateEnumTypeDefinition({ ...args, dataType });
  if (dataType instanceof MixinType) return this.generateMixinTypeDefinition({ ...args, dataType });
  if (dataType instanceof MappedType) return this.generateMappedTypeDefinition({ ...args, dataType });
  if (dataType instanceof ComplexType) return this.generateComplexTypeDefinition({ ...args, dataType });
  return '';
}
...
export async function generateEnumTypeDefinition(
  ...
  if (args.intent !== 'scope') throw new TypeError(`Can't generate EnumType for "${args.intent}" intent`);

  if (!dataType.name) throw new TypeError(`Name required to generate EnumType for "${args.intent}" intent`);

  let out = `enum ${dataType.name} {\n\t`;
  for (const [value, info] of Object.entries(dataType.values)) {
    // Print JSDoc
    let jsDoc = '';
    if (dataType.values[value].description) jsDoc += ` * ${dataType.values[value].description}\n`;

    if (jsDoc) out += `/**\n${jsDoc} */\n`;

    out +=
      `${info.key || value} = ` + (typeof value === 'number' ? value : "'" + String(value).replace("'", "\\'") + "'");
    out += ',\n\n';
  }
  return out + '\b}';
}
...
export async function generateComplexTypeDefinition(
  ...
  if (dataType.base) {
    ...
    const omitFields = [...dataType.own.fields.keys()].filter(k => dataType.base?.fields.has(k));
    const baseDef = omitFields.length ? `Omit<${base}, ${omitFields.map(x => "'" + x + "'").join(' | ')}>` : `${base}`;
    if (intent === 'scope') out += `extends ${baseDef} `;
    else {
      out += baseDef;
      if (!dataType.own.fields.size) return out;
      out += ' & ';
    }
  }
  ...
  for (const field of dataType.own.fields.values()) {
    ...
    if (field.fixed) {
      ...
      out +=
        (await this.resolveTypeNameOrDef({ file: args.file, dataType: field.type, intent: 'field' })) +
        `${field.isArray ? '[]' : ''};\n`;
    }
  }
...
export async function generateSimpleTypeDefinition(
  ...
  if (dataType.extendsFrom('boolean')) out += 'boolean';
  else if (dataType.extendsFrom('string')) out += 'string';
  else if (dataType.extendsFrom('number') || dataType.extendsFrom('integer')) out += 'number';
  else if (dataType.extendsFrom('date') || dataType.extendsFrom('datetime')) out += 'Date';
  else if (dataType.extendsFrom('datestring') || dataType.extendsFrom('datetimestring')) out += 'string';
  else if (dataType.extendsFrom('bigint')) out += 'bigint';
  else if (dataType.extendsFrom('object')) out += 'object';
  else out += 'any';
...
export async function generateMixinTypeDefinition(
  ...
  return (await Promise.all(dataType.types.map(t => this.resolveTypeNameOrDef({ file, dataType: t, intent }))))
...
export async function generateMappedTypeDefinition(
  ...
  const partial =
    dataType.partial === true || (Array.isArray(dataType.partial) && dataType.partial.length > 0)
      ? dataType.partial
      : undefined;
  if (!(pick || omit || partial)) return typeDef;
  let out = '';
