...
describe('MongoAdapter.prepareFilter', function () {
  ...
  describe('Convert Ast to mongo filter', function () {
    it('Should convert ComparisonExpression (=)', async () => {
      let out = MongoAdapter.prepareFilter(customers.normalizeFilter('givenName="Demons"')!);
      expect(out).toStrictEqual({givenName: 'Demons'});
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('rate=10')!);
      expect(out).toStrictEqual({rate: 10});
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('active=true')!);
      expect(out).toStrictEqual({active: true});
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('active=false')!);
      expect(out).toStrictEqual({active: false});
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('birthDate="2020-06-11T12:30:15"')!);
      expect(out).toStrictEqual({birthDate: '2020-06-11T12:30:15'});
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('deleted=null')!);
    ...
    it('Should convert ComparisonExpression (>)', async () => {
      const out = MongoAdapter.prepareFilter(customers.normalizeFilter('rate>5')!);
    ...
    it('Should convert ComparisonExpression (in)', async () => {
      let out = MongoAdapter.prepareFilter(customers.normalizeFilter('rate in [5,6]')!);
      expect(out).toStrictEqual({rate: {$in: [5, 6]}});
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('rate in 5')!);
    ...
    it('Should convert ComparisonExpression (like)', async () => {
      const out = MongoAdapter.prepareFilter(customers.normalizeFilter('givenName like "Demons"')!);
    ...
    it('Should convert ComparisonExpression (ilike)', async () => {
      const out = MongoAdapter.prepareFilter(customers.normalizeFilter('givenName ilike "Demons"')!);
    ...
    it('Should convert LogicalExpression (or)', async () => {
      const out = MongoAdapter.prepareFilter(customers.normalizeFilter('rate=1 or rate=2')!);
      expect(out).toStrictEqual({$or: [{rate: 1}, {rate: 2}]});
    });

    it('Should convert LogicalExpression (and)', async () => {
      const out = MongoAdapter.prepareFilter(customers.normalizeFilter('rate=1 and rate=2')!);
      expect(out).toStrictEqual({$and: [{rate: 1}, {rate: 2}]});
    });

    it('Should convert ParenthesesExpression', async () => {
      const out = MongoAdapter.prepareFilter(customers.normalizeFilter('(rate=1 or rate=2) and givenName = "Demons"')!);
    ...
    it('Should convert NegativeExpression', async () => {
      let out = MongoAdapter.prepareFilter(customers.normalizeFilter('not rate=1')!);
      expect(out).toStrictEqual({rate: {$not: {$eq: 1}}});
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('not rate=null')!);
      expect(out).toStrictEqual({$and: [{$ne: {rate: null}}, {rate: {$exists: true}}]});
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('not rate!=null')!);
      expect(out).toStrictEqual({$or: [{rate: null}, {rate: {$exists: false}}]});
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('not (rate=1 and active=true)')!);
      expect(out).toStrictEqual({
        $and: [
          {rate: {$not: {$eq: 1}}},
          {active: {$not: {$eq: true}}}
      ...
      out = MongoAdapter.prepareFilter(customers.normalizeFilter('not givenName like "Demons"')!);
      expect(out).toStrictEqual({
        givenName:
