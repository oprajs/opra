import {ComparisonExpression, IntegerFormat, OwoURL} from '../src';
...
describe('Parse url string', () => {
  ...
  it('Should parse path without domain', () => {
    const u = new OwoURL('/Person');
    expect(u.path.get(0)).toEqual({resource: 'Person'});
    expect(u.href).toStrictEqual('/Person');
  })

  it('Should parse path (percent encoded resource)', () => {
    const u = new OwoURL('https://anyuri.com/Hello%20World|%22%20%22');
  ...
  it('Should parse path with named resource key', () => {
    const u = new OwoURL('/Person|id=123');
  ...
  it('Should parse path with multiple named resource key', () => {
    const u = new OwoURL('/Person|mrn=123;y=2000');
  ...
  it('Should parse if resource key contains special chars', () => {
    const u = new OwoURL('/Person|id=' + encodeURIComponent('abc=/\\?# x'));
  ...
  it('Should throw if url is invalid', () => {
    expect(() => new OwoURL('Person|id=1;2')).toThrow('Invalid URL');
    expect(() => new OwoURL('http:Person|id=1;2')).toThrow('Invalid URL');
  })

  it('Should throw if mixing key:value keys and simple keys', () => {
    expect(() => new OwoURL('/Person|id=1;2')).toThrow('name:value pair required');
    expect(() => new OwoURL('/Person|1;id=1')).toThrow('name:value pair required');
    expect(() => new OwoURL('/Person|1;2')).toThrow('name:value pair required');
  })
  ...
  it('Should initialize with uri and prefix', () => {
    const u = new OwoURL('http://anyuri.com/service', '/service');
    expect(u.prefix).toStrictEqual('/service');
  })

  it('Should get origin', () => {
    const u = new OwoURL('http://anyuri.com:81/any/path');
