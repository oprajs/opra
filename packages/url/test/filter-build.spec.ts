...
describe('Building Filter', function () {
  ...
  it('Should $eq() create ComparisonExpression', () => {
    const x = $eq('a', 1);
    expect(x.type).toStrictEqual('ComparisonExpression');
    expect(x.op).toStrictEqual('=');
    expect(x.left.type).toStrictEqual('StringLiteral');
    // @ts-ignore
    expect(x.left.value).toStrictEqual('a');
    expect(x.right.type).toStrictEqual('NumberLiteral');
    // @ts-ignore
  ...
  it('Should $array() create ArrayExpression', () => {
    const x = $array('a', 2);
    expect(x.type).toStrictEqual('ArrayExpression');
    expect(x.items.length).toStrictEqual(2);
    expect(x.items[0].type).toStrictEqual('StringLiteral');
    expect(x.items[1].type).toStrictEqual('NumberLiteral');
  ...
  it('Should $arithmetic() create ArithmeticExpression', () => {
    ...
    expect(x.type).toStrictEqual('ArithmeticExpression');
  ...
  it('Should $paren() create ParenthesesExpression', () => {
    ...
    expect(x.type).toStrictEqual('ParenthesesExpression');
  ...
  it('Should $and() create LogicalExpression', () => {
    ...
    expect(x.type).toStrictEqual('LogicalExpression');
  ...
  it('Should wrap entry values to Ast objects', () => {
    ...
    expect(x.items[0].type).toStrictEqual('StringLiteral');
    expect(x.items[1].type).toStrictEqual('NumberLiteral');
    expect(x.items[2].type).toStrictEqual('BooleanLiteral');
    expect(x.items[3].type).toStrictEqual('NumberLiteral');
    expect(x.items[4].type).toStrictEqual('DateLiteral');
    expect(x.items[5].type).toStrictEqual('NullLiteral');
    expect(x.items[6].type).toStrictEqual('ArrayExpression');
  })
