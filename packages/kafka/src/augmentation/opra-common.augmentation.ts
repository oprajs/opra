  ...
  interface RpcOperationDecorator {
    Kafka(
      config:
        ...
        | (() => KafkaAdapter.OperationOptions | Promise<KafkaAdapter.OperationOptions>),
    ): this;
  }
}
...
classes.RpcOperationDecoratorFactory.augment((decorator: any, decoratorChain) => {
  decorator.Kafka = (config: KafkaAdapter.OperationOptions): any => {
    decoratorChain.push((_: RpcOperation.Metadata, target: Object, propertyKey: string | symbol): void => {
