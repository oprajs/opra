...
> extends Observable<T> {
  ...
  constructor(backend: HttpBackend, init?: HttpBackend.RequestInit) {
    super(subscriber => {
      ...
      const handleEvent = (event: HttpEvent) => {
      ...
      /* Bundled requests never talk to the network themselves — they wait
         for whatever HttpBundleObservable pushes into this subject */
      const source: Observable<HttpEvent> =
        context._bundleEvents ||
  ...
  get requestId(): string {
    return this[kContext].requestId;
  }
   ...
   */
  _bindBundle(): void {
    this[kContext]._bundleEvents ??= new ReplaySubject<HttpEvent>();
  }
  ...
  _finalize(response: HttpResponse<any>): void {
    const subject = this[kContext]._bundleEvents;
    if (!subject) return;
    const request = this[kContext];
    subject.next({
  ...
  _finalizeError(error: unknown): void {
    this[kContext]._bundleEvents?.error(error);
  }
  ...
  observe(observe: HttpObserveType): Observable<any> {
    ...
    const cloned = new HttpRequestObservable<
      T,
      TBody,
      TRequestOptions,
      TResponseExt
    >(this[kBackend], this[kContext]);
