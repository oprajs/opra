...
import { HttpExecutionContextHost } from '../../../src/adapter/http/http-execution-context.host.js';
...
describe('OpraHttpAdapter', function () {
  ...
  class TestHttpAdapter extends OpraHttpAdapter<HttpExecutionContext> {
    static async create(doc: ApiDocument, options?: OpraHttpAdapter.Options): Promise<OpraHttpAdapter> {
  ...
  describe('parse request', function () {
    ...
    it('Should parse "CollectionGetQuery" query', async () => {
      ...
      const requestContext = await adapter.parse(executionContext) as QueryRequestContext;
      expect(requestContext).toBeDefined();
      expect(requestContext).toBeInstanceOf(QueryRequestContext);
      const query = requestContext.query as any;
