...
import { SingletonCreateQuery } from '@opra/core';
...
describe('SingletonCreateQuery', function () {
  let api: ApiDocument;
  ...
  it('Should create query', async () => {
    ...
    const query = new SingletonCreateQuery(resource, data);
  ...
  it('Should create query with "pick" option', async () => {
    const resource = api.getSingleton('BestCustomer');
    const query = new SingletonCreateQuery(resource, data, {pick: ['address.city']});
    expect(query.pick).toStrictEqual(['address.city']);
  })

  it('Should normalize field names in "pick" option', async () => {
    const resource = api.getSingleton('BestCustomer');
    let query = new SingletonCreateQuery(resource, data, {pick: ['givenname', 'GENDER', 'AdDRess.CIty']});
    expect(query.pick).toStrictEqual(['givenName', 'gender', 'address.city']);
    query = new SingletonCreateQuery(resource, data, {pick: ['address', 'address.city']});
    expect(query.pick).toStrictEqual(['address']);
    query = new SingletonCreateQuery(resource, data, {pick: ['address.city', 'address']});
    expect(query.pick).toStrictEqual(['address']);
  })

  it('Should validate if fields in "pick" option are exist', async () => {
    const resource = api.getSingleton('BestCustomer');
    expect(() => new SingletonCreateQuery(resource, data, {pick: ['address.xid']}))
        .toThrow('Unknown element "address.xid"');
  })

  it('Should allow unknown fields in "pick" option if additionalFields set to "true"', async () => {
    const resource = api.getSingleton('BestCustomer');
    const query = new SingletonCreateQuery(resource, data, {pick: ['notes.add1', 'notes.add2.add3']});
    expect(query.pick).toStrictEqual(['notes.add1', 'notes.add2.add3']);
  })

  it('Should create query with "omit" option', async () => {
    const resource = api.getSingleton('BestCustomer');
    const query = new SingletonCreateQuery(resource, data, {omit: ['address.city']});
    expect(query.omit).toStrictEqual(['address.city']);
  })

  it('Should normalize field names in "omit" option', async () => {
    const resource = api.getSingleton('BestCustomer');
    let query = new SingletonCreateQuery(resource, data, {omit: ['givenname', 'GENDER', 'AdDRess.CIty']});
    expect(query.omit).toStrictEqual(['givenName', 'gender', 'address.city']);
    query = new SingletonCreateQuery(resource, data, {omit: ['address', 'address.city']});
    expect(query.omit).toStrictEqual(['address']);
    query = new SingletonCreateQuery(resource, data, {omit: ['address.city', 'address']});
    expect(query.omit).toStrictEqual(['address']);
  })

  it('Should validate if fields in "omit" option are exist', async () => {
    const resource = api.getSingleton('BestCustomer');
    expect(() => new SingletonCreateQuery(resource, data, {omit: ['address.xid']}))
        .toThrow('Unknown element "address.xid"');
  })

  it('Should allow unknown fields in "omit" option if additionalFields set to "true"', async () => {
    const resource = api.getSingleton('BestCustomer');
    const query = new SingletonCreateQuery(resource, data, {omit: ['notes.add1', 'notes.add2.add3']});
    expect(query.omit).toStrictEqual(['notes.add1', 'notes.add2.add3']);
  })

  it('Should create query with "include" option', async () => {
    const resource = api.getSingleton('BestCustomer');
    const query = new SingletonCreateQuery(resource, data, {include: ['address.city']});
    expect(query.include).toStrictEqual(['address.city']);
  })

  it('Should normalize field names in "include" option', async () => {
    const resource = api.getSingleton('BestCustomer');
    let query = new SingletonCreateQuery(resource, data, {include: ['givenname', 'GENDER', 'AdDRess.CIty']});
    expect(query.include).toStrictEqual(['givenName', 'gender', 'address.city']);
    query = new SingletonCreateQuery(resource, data, {include: ['address', 'address.city']});
    expect(query.include).toStrictEqual(['address']);
    query = new SingletonCreateQuery(resource, data, {include: ['address.city', 'address']});
    expect(query.include).toStrictEqual(['address']);
  })

  it('Should validate if fields in "include" option are exist', async () => {
    const resource = api.getSingleton('BestCustomer');
    expect(() => new SingletonCreateQuery(resource, data, {include: ['address.xid']}))
        .toThrow('Unknown element "address.xid"');
  })

  it('Should allow unknown fields in "include" option if additionalFields set to "true"', async () => {
    const resource = api.getSingleton('BestCustomer');
    const query = new SingletonCreateQuery(resource, data, {include: ['notes.add1', 'notes.add2.add3']});
