...
export function collectionDeleteTests(args: { client: OpraTestClient }) {
  describe('Collection:deleteOne', function () {
    ...
    it('Should delete instance', async () => {
      const resp = await args.client.collection('Customers').delete(1001).getResponse();
      resp.expect.toSuccess().toReturnOperationResult().toBeAffected();
      await expect(() => args.client.collection('Customers').get(1001).toPromise()).rejects.toThrow('422');
    });
  });
}
