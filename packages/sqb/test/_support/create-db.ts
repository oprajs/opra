...
export async function createTestSchema() {
  const schema = process.env.DB_SCHEMA || 'opra_test';
...
async function createTestData(connection: Connection, schema: string) {
  let lines: string[] = [];
...
export function enumValue<T extends string | number>(enumVal: Record<string, T> | T[]): T {
